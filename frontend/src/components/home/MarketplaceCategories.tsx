...
export default function MarketplaceCategories() {
  ...
  return (
    <section ref={containerRef} className="pt-24 pb-12 bg-white relative overflow-hidden">
      {/* Background Orbs */}
      <div className="absolute top-0 right-1/4 w-[500px] h-[500px] bg-primary-600/5 rounded-full blur-[110px] pointer-events-none" />
      <div className="absolute bottom-0 left-1/4 w-[500px] h-[500px] bg-accent-cyan/5 rounded-full blur-[110px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <motion.div style={{ y }} className="text-center mb-20">
          ...
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-slate-900 mb-6 tracking-tight font-outfit">
            Curate Your Perfect <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary-600 via-purple-600 to-pink-500">Experience</span>
          </h2>
          <p className="text-slate-500 max-w-2xl mx-auto text-base md:text-lg leading-relaxed font-outfit">
          ...
          {categories.map((cat, i) => (
            <Link key={cat.id} href={`/marketplace?category=${cat.id}`} className="h-full">
              <motion.div
                ...
                className="group relative p-6 rounded-2xl bg-slate-50/50 border border-slate-100 hover:border-slate-200/80 hover:bg-white transition-all duration-300 overflow-hidden h-full flex flex-col items-center text-center shadow-sm hover:shadow-xl hover:shadow-slate-200/40 cursor-pointer"
              >
                ...
                <h3 className="text-base md:text-lg font-bold text-slate-800 group-hover:text-primary-600 transition-colors duration-300 mb-1.5 font-outfit">{cat.title}</h3>
                <p className="text-xs text-slate-400 font-medium font-outfit tracking-wide">{cat.desc}</p>
